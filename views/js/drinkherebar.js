...
function highlightnonAlcoholic(idTable, bShowNonA) {
    // if bShowNonA is true, then we're highlighting nonAlcoholic
    //	drinks, otherwise we're unhighlighting them.
    var i = 0;
    var oTable = document.getElementById(idTable);
    var oTBODY = oTable.getElementsByTagName('tbody')[0];
    var aTRs = oTBODY.getElementsByTagName('tr');
    // walk through each of the table rows and see if it has a 
    // "nonAlcoholic" attribute on it.
    for (i = 0; i < aTRs.length; i++) {
        if (aTRs[i].getAttribute('nonAlcoholic') == "true") {
            if (bShowNonA) {
